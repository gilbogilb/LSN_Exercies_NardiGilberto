...
// contribution to energy and virial
  ...
  //ofstream raw("output/raw.dat",ios::app);

  timestep  = timestep + 1.0;

  //raw << walker[iv]/(double)npart + vtail << " "
...
//g(r)
    for(int i=igofr; i<n_props; ++i){
      ...
      norm = rho*npart*(4./3.)*pi*(pow(r+bin_size,3.) - pow(r,3.));
      gdir = blk_av[i]/norm/blk_norm;

      glob_av[i]  += gdir;
      glob_av2[i] += gdir*gdir;
      //print to file
      Gofr << gdir << " "; //block average
      if(iblk == nblk-1) { //final estimate of g(r)
        Gave << r << " " << glob_av[i]/(double)iblk << " " 
             << Error(glob_av[i],glob_av2[i],iblk) << endl;
      }
    }
