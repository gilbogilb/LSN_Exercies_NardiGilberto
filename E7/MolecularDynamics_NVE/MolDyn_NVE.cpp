...
int main(){ 
  ...
  for(int i=0; i<B; ++i){
    ...
    Averages();
    if((B*N+k)%iprint==0) cout << "done step " << B*N+k << endl;
  }
...
void Averages(){
  ...
  //deals with data blocking (here implemented for g(r))
  ofstream Gave("outputs/output.gave.0",ios::app),
           Gofr("outputs/output.gofr.0",ios::app);
  double r, gdir, norm;
  
  for (int i=0; i<nbins; ++i){
    r = double(i)*bin_size;
    norm = rho*npart*(4./3.)*M_PI*(pow(r+bin_size,3.) - pow(r,3.));
    gdir = gofr[i]/norm/double(N); 

    global_av[i] = (global_av[i]*iblk+gdir)/double(iblk+1);
    global_av2[i] = (global_av2[i]*iblk+ gdir*gdir)/double(iblk+1);

    Gofr << gdir << " "; //block average
    if(iblk == B-1) { //final estimate of g(r)
      Gave << r << " " << global_av[i] << " " 
           << error(global_av[i],global_av2[i],iblk) << endl;
    }
    gofr[i]=0;
  }
  Gofr << endl;
...
double error(double mean, double mean2, int iblk)
{
    if( iblk == 1 ) return 0.0;
    else return sqrt((mean2 - mean*mean)/(double)(iblk-1));
}
/****************************************************************
*****************************************************************
    _/    _/  _/_/_/  _/       Numerical Simulation Laboratory
