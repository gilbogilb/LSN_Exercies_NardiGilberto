...
class GeneticTSP : public TSP{
    ...
    public:
        GeneticTSP(int pop_size, int cities, char cities_distr, std::string seed_filename); //char: C on a circumference, S over a square
        ~GeneticTSP(){}
        ...
        //set members
        void SetElement(std::vector<int> el, int index) {m_population.at(index)=el;}
...
class SimulatedAnnealingTSP: public GeneticTSP { //I need the genetic operators

    public:
        SimulatedAnnealingTSP(int cities, double T0,char cities_distr, std::string seed_filename): GeneticTSP(1,cities,cities_distr,seed_filename), m_beta(1./T0) {}
        ~SimulatedAnnealingTSP(){}

        void Propose();
        void SetT(double T) {m_beta = 1./T;}
        double GetBeta()const {return m_beta;}
        std::vector<int> GetSolution()const{return m_population.at(0);}
...
TSP::L2cost(std::vector<int>& indexes){
    ...
    if(!m_ready){
        std::cout << "you have called L2cost() without having generated a distribution of cities! (m_ready==false). End" << std::endl;
        exit(1);
    }

    if(indexes.size()!=m_cities.size()){
        std::cout << std::endl << "m_cities: ";

        //for(auto it=m_cities.begin(); it!=m_cities.end(); ++it){
        //    std::cout << (*it).GetX() << " " << (*it).GetY() << " ";}
        std::cout << "error: proposed permutation vector size and city size are not equal\n" << indexes.size() << " " << m_cities.size() << std::endl;
        //std::cout << "indexes: " ;
        //for(auto it=indexes.begin(); it!= indexes.end(); ++it) std::cout << *it << " ";
        std::cout << std::endl << "m_cities: ";

        for(auto it=m_cities.begin(); it!=m_cities.end(); ++it){
            std::cout << "test " << (*it).GetX() << " ";}
...
GeneticTSP::GeneticTSP(int pop_size, int cities, char cities_distr, std::string seed_filename): TSP(cities), m_comparator(this) {

    m_rnd.Initialize(1,"Primes",seed_filename);
...
void
SimulatedAnnealingTSP::Propose(){
    std::vector<int> prop = m_population.at(0);

    //randomly choose a mutation
    MutationReverse(prop);

    double L2old = L2cost(m_population.at(0)),
           L2new = L2cost(prop);

    if(L2new<L2old) m_population.at(0) = prop;
    else if(m_rnd.Rannyu()<exp(-m_beta*(L2new-L2old))) m_population.at(0) = prop;

    MutationSwap(prop);
    L2old = L2cost(m_population.at(0)),
    L2new = L2cost(prop);

    if(L2new<L2old) m_population.at(0) = prop;
    else if(m_rnd.Rannyu()<exp(-m_beta*(L2new-L2old))) m_population.at(0) = prop;

    MutationSwapBlock(prop);
    L2old = L2cost(m_population.at(0)),
    L2new = L2cost(prop);
...
GeneticTSP::MutationSwap(std::vector<int>& starter){
    ...
    int i = (int)m_rnd.Rannyu(1.,starter.size()),
        j = (int)m_rnd.Rannyu(1.,starter.size()),
        tmp;
...
GeneticTSP::MutationReverse(std::vector<int>& starter){
    ...
    for(int i=0; i<(last-first)/2; ++i){
        tmp = starter.at(i+first);
        starter.at(i+first) = starter.at(last-i);
...
GeneticTSP::Select() {
    ...
    do{
        ...
        /*std::cout << "ready to calculate cost of m_population.at(random_index) which is ";
        for(auto it=m_population.at(random_index).begin(); it!=m_population.at(random_index).end();++it)
            std::cout << *it << " ";
        std::cout << std::endl;*/
