...
DataBlockingTool::m_Write() {
	...
	if(out.good()){
		...
		out << m_prog_mean << " " << GetError() << std::endl;
	}
	else{
		std::cerr << "error: broken ofstream in Write(). end\n";
		exit(-1);
	}

}

double DataBlockingTool::GetError() const {
	if(m_N<2)
		return 0.;
	else
		return sqrt((m_prog_mean2-m_prog_mean*m_prog_mean)/static_cast<double>(m_N-1));
}
